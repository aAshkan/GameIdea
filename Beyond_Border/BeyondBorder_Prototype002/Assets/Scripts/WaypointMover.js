...
public var suspended: boolean = false;	//on normal conditions, this should be false
   	...
   	if(StartFromNearestWaypoint)
  	   {
  	     ...
  	     //Test vars
  	     var testLength = waypointsHolder.waypoints.length;
  	     //Debug.LogFormat("++++++++++++++++++++++++++++++++++++++++++ ", testLength);
  	    // Debug.Log ("message22 " + testLength);
  	     //End test vars
  	        // Debug.Log("test = " + testLength.ToString());
  	     
  	     for (var i:int = 0; i < waypointsHolder.waypoints.Length; i++)
  	      {
  	         //Debug.Log("Length = " + waypointsHolder.waypoints.Length);
...
function Update () 
{
 ...
 if (!suspended  &&  !collisionPrevented  &&  currentWaypoint >= 0  &&  delayTillTime < Time.time && Input.GetKey(KeyCode.Space) )//&& TurnControl.isAiTurn) // && !isPlayerTurn) 
  {
...
function isPlayer ()
{	
	TurnControl.isPlayerTurn = true;
	TurnControl.isAiTurn = false;
}
//------------------------------------------------------------------------------------
//Activate Ai's Turn
function isAi ()
